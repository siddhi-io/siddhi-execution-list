...
@Extension(
        name = "lastIndexOf",
        namespace = "list",
        description = "Function returns the index of the given value.",
        parameters = {
                @Parameter(name = "list",
        ...
        returnAttributes =
                @ReturnAttribute(
                        description = "Returns index of the value if present, else -1 otherwise",
                        type = DataType.INT
                ),
        examples =
                @Example(
                        syntax = "list:lastIndexOf(stockSymbols. `IBM`)",
...
public class LastIndexOfFunctionExtension extends FunctionExecutor<State> {
    ...
    protected Object execute(Object[] data, State state) {
        if (data[0] instanceof List) {
            return ((List) data[0]).lastIndexOf(data[1]);
        }
