...
public class SetFunctionExtensionTestCase {
    ...
    @Test
    public void testSetFunctionExtension() throws InterruptedException {
        log.info("SetFunctionExtension TestCase");
        SiddhiManager siddhiManager = new SiddhiManager();

        String inStreamDefinition = "\ndefine stream inputStream (symbol string, price long, volume long);";
        String query = ("@info(name = 'query1') from inputStream select symbol,price, "
                + "list:create('TEMP') as tmpList insert into tmpStream;"
                + "@info(name = 'query2') from tmpStream  "
                + "select symbol,price,tmpList, list:setValue(tmpList, 0, symbol) as newList"
    ...
    public void testSetFunctionExtension1() {
        log.info("SetFunctionExtension TestCase with test attributeExpressionExecutors length");
        SiddhiManager siddhiManager = new SiddhiManager();
        ...
        String query = ("@info(name = 'query1') from inputStream select symbol,price, "
                ...
                + "@info(name = 'query2') from tmpStream  "
                + "select symbol,price,tmpList, list:setValue(tmpList) as newList"
                + " insert into outputStream;"
        );
