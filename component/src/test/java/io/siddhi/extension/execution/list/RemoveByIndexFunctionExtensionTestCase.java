...
/**
 * Test case for list:removeByIndex.
 */
public class RemoveByIndexFunctionExtensionTestCase {
    private static final Logger log = Logger.getLogger(RemoveByIndexFunctionExtensionTestCase.class);
    ...
    public void testRemoveFunctionExtension() throws InterruptedException {
        ...
        String query = ("@info(name = 'query1') from inputStream select symbol,price, "
                + "list:create('ABCD') as tmpList insert into tmpStream;"
                + "@info(name = 'query2') from tmpStream  select symbol,price,tmpList, " +
                "list:add(tmpList,symbol) as list1"
                + " insert into outputStream;"
                + "@info(name = 'query3') from outputStream  select list1, list:removeByIndex(list1, 0) as list2"
    ...
    public void testRemoveFunctionExtension1() throws InterruptedException {
        ...
        String query = ("@info(name = 'query1') from inputStream select symbol,price, "
                ...
                + "@info(name = 'query3') from outputStream  select list1, list:removeByIndex(list1) as list2"
                + " insert into outputStream2;"
        );
